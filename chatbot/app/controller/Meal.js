...
const MealModel = require('../model/Meal') //model/Meal에 있는 get함수를 사용하겠다
...
Meal.update = async function () {  //지금은 없어도될듯
  try {
    ...
    await MealModel.update(data) //없어도될듯
...
Meal.get = async function (date) { //지금 controll에서 핵심부분
    const row = await MealModel.get(date == 'day') //type이 'today'라는 투플을검색해서 row에 삽입
    if (row && row.date && row.info) {
      return month1 + '월' + row.date + '일 중도식당' + '\n\n' + row.info 
